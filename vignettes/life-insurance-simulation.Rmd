...
Now that we have male and female life tables lets assign each insuree to an `diehard3000::Life` object.  The `Life` object identifies the `Life`, age, and
other policy characteristics appropriate to the individul policy.  Once the `Life` object is defined we can run the simulations.
...
Each element in our `hold` list is an object of class `Life`.  We can simulate the future life and present value of the benefits for any of these `Life` objects individually like so:
...
and we can make similar plots to those for the single `Life` object:
