...
The following code shows how to deine a `LifeTable` object:
...
# make examples using the Insuree object inwhich the
# Insuree's life insurance policy lasts until the end of the
# life table.  Any age greater than the Insuree's current age
# could be used in place of max_m and max_f as defined below
max_m <- max(qx_male@x)
...
Each element in our `hold` list above is an object of class `Insuree`.  We can simulate the future life and present value of the benefits for any of these `Insuree` objects individually like so
...
Simulating one life insurance policy is not that interesting.  The real power of the `insuree` package is to group a several policies together and simulate them as one group.  By simulating the group we can come up with confidence levels for the present value of future benefit payments that would otherwise be very difficult to calculate.

We run `r n` observations of each individual's future life expectancy.  We assume each individual future life expectancy is independent and identically distributed.  We discount the death benefit, assuming a `r rate * 100` % interest rate, for the number of years the insuree survives.
